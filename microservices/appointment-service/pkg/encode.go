...
func encode[T any](w http.ResponseWriter, status int, response T) {
	encodeWithContentType(w, status, response, server.ApplicationJSON)
}

func encodeError(w http.ResponseWriter, err *ApiError) {
	encodeWithContentType(w, err.Status, err.ErrorDetail, server.ApplicationProblemJSON)
}
...
func encodeWithContentType[T any](
	...
	w.Header().Set(server.ContentType, contentType)
	w.WriteHeader(code)
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		...
		http.Error(w, server.EncodingError, http.StatusInternalServerError)
	}
}
...
func decode[T any](w http.ResponseWriter, r *http.Request) (T, error) {
	...
	if err != nil {
		...
		case err.Error() == largeBodyErrorStr:
			return dst, fmt.Errorf("body must not be larger than %d bytes", server.MaxBytes)
