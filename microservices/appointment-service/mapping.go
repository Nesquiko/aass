...
func (a appointmentServer) fetchAvailableResources(
	ctx context.Context,
	dateTime time.Time,
) (
	*resourceapi.AvailableResources,
	*server.ApiError,
) {
	params := resourceapi.GetAvailableResourcesParams{DateTime: dateTime}
	resResp, err := a.resourceApi.GetAvailableResourcesWithResponse(ctx, &params)
	if err != nil {
		slog.Error(
			...
			err.Error(),
			"where",
			"fetchAvailableResources api call",
		)
		return nil, server.InternalServerError()
	}
	if resResp.JSON200 == nil {
		slog.Error(
			"failed to get available resources",
			"status",
			resResp.StatusCode(),
			"body",
			string(resResp.Body),
	...
	return resResp.JSON200, nil
}

func (a appointmentServer) getSelectedResources(
	req api.AppointmentDecision,
	available *resourceapi.AvailableResources,
) []Resource {
	selected := make([]Resource, 0, 3)
	if req.Facility != nil {
		for _, f := range available.Facilities {
			if f.Id == *req.Facility {
	...
	if req.Equipment != nil {
		for _, e := range available.Equipment {
			if e.Id == *req.Equipment {
				selected = append(selected, Resource{
					Id:   e.Id,
					Name: e.Name,
